...
| Browser IME diagnostics and Atlassian Confluence live repros | `browser` | `nix develop .#browser --command -- make chrome-ime-repro` |
...
The `browser` shell is for Chrome and Firefox IME diagnostics and Atlassian
Confluence live repros. It includes the default app/Xcode tools plus Node.js/npm
for Playwright and Selenium observers, and geckodriver for the Firefox local
fixture. `tools/chrome_ime_repro.nu`, `tools/firefox_ime_repro.nu`, and
`tools/atlassian_confluence_repro.nu` install and run their browser observers
from this shell. It also intentionally avoids Nix Swift because the browser HID
drivers import macOS frameworks and are compiled with Xcode's `xcrun swiftc`.
